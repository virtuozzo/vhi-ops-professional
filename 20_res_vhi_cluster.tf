...
resource "openstack_compute_instance_v2" "vhi-mn_nodes" {
  ...
  dynamic "network" {
    for_each = local.enable_vm_public ? [1] : []
    content {
  ...
  config_drive = true
  user_data = templatefile(local.node_cloud_init,
    merge(
      {
      ...
      local.enable_vm_public ? { vm_public_ip = "10.100.0.1${count.index + 1}" } : {}
    )
  )
}
...
resource "openstack_compute_instance_v2" "vhi-worker_nodes" {
  ...
  user_data = templatefile(local.node_cloud_init,
    merge(
      ...
      local.enable_vm_public ? { vm_public_ip = "10.100.0.1${count.index + 4}" } : {}
    )
  )
}
